...
import { useMyObjects, type CatalogObjectRow } from '../../state/useMyObjects';
...
const CatalogCard = ({ p }: { p: CatalogObjectRow }) => {
  const navigate = useNavigate();
  return (
...
const CatalogCardMobile = ({ p }: { p: CatalogObjectRow }) => {
  const navigate = useNavigate();
  return (
    <a
      href={`/${p.code}`}
    ...
    >
      <div style={{ width: 64, height: 64, flex: 'none', overflow: 'hidden', background: '#eae7dc' }}>
        {p.cover_url && <img src={p.cover_url} alt={p.title} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
      </div>
...
export const CatalogPage = () => {
  ...
  const { objects, loading, refetch } = useMyObjects();
  const [modalOpen, setModalOpen] = useState(false);

  if (!user) return null;
  const heading = user.isAdmin ? 'Все объекты в системе' : 'Ваши объекты';
      ...
      <div style={{ minHeight: '100vh', background: SYS.paper, color: SYS.ink, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto' }} className="sys-modal-scroll">
          {loading ? (
            ...
            objects.map((p) => <CatalogCardMobile key={p.code} p={p} />)
          )}
        </div>

        {!loading && !user.isAdmin && objects.length === 1 && (
    ...
    <div style={{ width: '100%', minHeight: '100vh', background: SYS.bg, color: SYS.ink }}>
      ...
      <div style={{ padding: '40px 48px 56px' }}>
        ...
        {!loading && objects.length === 0 && (
          <div style={{ border: `1px dashed ${SYS.line}`, background: SYS.paper, padding: '72px 40px', textAlign: 'center' }}>
            ...
            <h2 style={{ margin: '0 0 8px', fontSize: 22, fontWeight: 500, letterSpacing: '-0.008em' }}>
              {user.isAdmin ? 'В системе пока нет объектов' : 'Пока нет доступных объектов'}
            </h2>
        ...
        {!loading && objects.length > 0 && (
          <>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 20 }}>
              {objects.map((p) => <CatalogCard key={p.code} p={p} />)}
            </div>
      ...
      {modalOpen && (
        <NewProjectModal
          ...
          onCreate={() => { refetch(); setModalOpen(false); }}
        />
      )}
    </div>
  );
