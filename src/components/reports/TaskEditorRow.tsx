...
import { SUBPROJECTS, type ReportTask } from '../../mocks/reports';

const MAX_PHOTOS = 4;

export const TaskEditorRow = ({
  task, n, allowDeskType, onChange, onRequestDelete, nameError,
}: {
  task: ReportTask;
  n: number;
  ...
  const addFiles = (files: FileList | null) => {
    ...
    const added = picked.map((f) => ({ id: `${Date.now()}-${f.name}-${Math.random()}`, url: URL.createObjectURL(f) }));
    onChange({ photos: [...task.photos, ...added] });
  };
