...
// day-of-year mapping (non-leap-safe enough for a static mock, ported as-is)
const CUM = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365];
const toDay = (d: string | null | undefined) => {
  if (!d) return null;
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: SYS.muted, letterSpacing: '0.08em', marginBottom: 6 }}>
        <span>{s.start} · старт</span>
        <span>{s.handover} · сдача</span>
      </div>
      <div style={{ position: 'relative', height: 10, marginTop: 22 }}>
        ...
        {xMeet != null && (
          <Fragment>
            <div style={{ position: 'absolute', left: `${xMeet}%`, bottom: 18, transform: 'translateX(-50%)', fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: SYS.red, letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{s.meeting}</div>
        ...
        {(s.extraMarkers || []).map((d, i) => {
          ...
          return x == null ? null : (
            <Fragment key={i}>
              <div style={{ position: 'absolute', left: `${x}%`, bottom: 18, transform: 'translateX(-50%)', fontFamily: "'JetBrains Mono', monospace", fontSize: 9.5, color: SYS.red, letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{d}</div>
              <div title={d} style={{ position: 'absolute', left: `${x}%`, top: '50%', transform: 'translate(-50%,-50%)', width: 14, height: 14, borderRadius: '50%', background: SYS.paper, border: `2px solid ${SYS.red}` }} />
            </Fragment>
          );
        })}
        {xToday != null && (
      ...
      {xToday != null && (
        <div style={{ marginTop: 4, position: 'relative', height: 14 }}>
          <span style={{ position: 'absolute', left: `${xToday}%`, transform: 'translateX(-50%)', fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: SYS.red, letterSpacing: '0.08em' }}>сегодня · {s.today}</span>
        </div>
      )}
    </div>
  );
